...
#    def notices_type_for(self, user, archived=False, unseen=None, on_site=None, sent=False, notice_type=None):
#        if sent:
#            lookup_kwargs = {"sender": user}
#        else:
#            lookup_kwargs = {"recipient": user}
...
#    def type_unseen_count_for(self, recipient, notice_type, **kwargs):
#        """
#        returns the number of unseen notice for friend request
#        """
#        return self.notices_type_for(recipient, unseen=True, notice_type, **kwargs).count()
...
class EmailTemplate(models.Model):
    ...
    recipients = models.CharField(max_length=200, null=True, blank=True)
...
def send_now(users, label, extra_context=None, on_site=True, sender=None):
    ...
    notification.send(user, 'friend_invite_sent', {
        'spam': 'eggs',
        'foo': 'bar',
    )
    ...
    for user in users:
        ...
        # remove the send mail function since we didn't use it for our site
