...
class BootstrapMixin(object):
    ...
    def render_field(self, field):
        ...
        if type(field) == dict and 'label' in field and 'fields' in field:
            ctx_fields = []
            ctx = {'label': field['label'], 'fields': ctx_fields, 'errors': ''}
            field_list = field['fields']
            
            
            for _field in field_list:
                field_instance = self.fields[_field]
                
                if not field_instance.widget.attrs.get('placeholder') and\
                field_instance.help_text:
                    field_instance.widget.attrs['placeholder'] = field_instance.help_text            
                
                bf = forms.forms.BoundField(self, field_instance, _field)
                if bf.errors:
                ...
                ctx_fields.append(field_hash)

            template = select_template([os.path.join(self.template_base, 'field_group_inline.html'),])
        ...
        """
            Inline field set
            {'label': blah, 'fields': ['field1', 'field2']}
        """
