...
function bigformCheck(thefield){
	...
	if(isEmpty(thefield.value)){
		...
		if(thefield.name != "firstname" && thefield.name != "lastname" && thefield.name != "password1" && thefield.name != "password2"){
			return;
		}
	}
	switch(thefield.name){
		...
		case 'email':
			// Email invalid error
			if (mailCheck(thefield, bigValid, bigError, "Stella can't reach you there") && !isDefault($('input[name=email_again]')[0])){
				// Email mismatch error
				validCheck(thefield, $('input[name=email_again]')[0], bigValid, bigError, "Your email address doesn't match");
			}
			break;
		case 'email_again':
			// Email invalid error
			...
			break;
		case 'password1':
			if(!isDefault($('input[name=password2]')[0])){
				// Password mismatch error
				validCheck(thefield, $('input[name=password2]')[0], bigValid, bigError, "Your password doesn't match");
			}
			break;
		case 'password2':
			if(!isDefault($('input[name=password1]')[0])){
				// Password mismatch error
				validCheck(thefield, $('input[name=password1]')[0], bigValid, bigError, "Your password doesn't match");
			}
			break;
		default:
			...
			if(mailCheck($('input[name=email]')[0], bigValid, bigError, "Stella can't reach you there")){
				if(filledCheck($('input[name=firstname]')[0], bigValid, bigError, "Stella wants to know you on a first name basis")){
					if(filledCheck($('input[name=lastname]')[0], bigValid, bigError, "Stella wants to know you on a last name basis")){
						if(validCheck($('input[name=email]')[0], $('input[name=email_again]')[0], bigValid, bigError, "Your email email doesn't match")){
							validCheck($('input[name=password1]')[0], $('input[name=password2]')[0], bigValid, bigError, "Your email password doesn't match");
						}
					}
				}
			}
